 ...
 * so visitors who arrive from YouTube stay on the blog. Shows the front-end
 * and the bundle tier only — the bundle URL is provided per deal during
 * ingest.
 */
export function BlogAffiliateCta({
  ...
  const [mode, setMode] = useState<"fe" | "bundle">("fe");
  ...
  const openModal = (url: string, m: "fe" | "bundle") => {
    setSalesUrl(url);
    setMode(m);
    setOpen(true);
  };
